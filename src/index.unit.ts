...
module.exports = ({ name, redis }) => {
  ...
  describe(name, () => {
    beforeEach(() => {
      ...
      self.loadFn.withArgs(sinon.match([1, 2])).returns(
        Promise.resolve({
          ball: 'bat'
        })
      );
      ...
      self.userLoader = () =>
        new DataLoader(keys => Promise.map(keys, self.loadFn), {
          cache: false
        });

      return Promise.map(
        _.keys(self.data).concat(['{"a":1,"b":2}', '[1,2]']),
        k => rDel(`${self.keySpace}:${k}`)
      ).then(() => {
        self.loader = new RedisDataLoader(self.keySpace, self.userLoader());
        self.noCacheLoader = new RedisDataLoader(
          self.keySpace,
          self.userLoader(),
          {
            cache: false
    ...
    describe('load', () => {
      it('should load json value', () =>
        self.loader.load('json').then(data => {
      ...
      it('should require key', () =>
        expect(self.loader.load()).to.be.rejectedWith(TypeError));
    ...
    describe('loadMany', () => {
      it('should load multiple keys', () =>
        self.loader.loadMany(['json', 'null']).then(results => {
      ...
      it('should handle empty array', () =>
        self.loader.loadMany([]).then(results => {
          expect(results).to.deep.equal([]);
        }));
      it('should require array', () =>
        expect(self.loader.loadMany()).to.be.rejectedWith(TypeError));
    ...
    describe('prime', () => {
      ...
      it('should require key', () =>
        expect(
          self.loader.prime(undefined, {
            new: 'value'
          })
        ).to.be.rejectedWith(TypeError));
      it('should require value', () =>
        expect(self.loader.prime('json')).to.be.rejectedWith(TypeError));
    ...
    describe('clear', () => {
      ...
      it('should require a key', () =>
        expect(self.loader.clear()).to.be.rejectedWith(TypeError));
    });
