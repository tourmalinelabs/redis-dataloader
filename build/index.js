...
module.exports = fig => {
    ...
    const rSetAndGet = (keySpace, key, rawVal, opt) => toString(rawVal, opt).then(val => new Promise((resolve, reject) => {
    ...
    const rGet = (keySpace, key, opt) => new Promise((resolve, reject) => redis.get(makeKey(keySpace, key, opt.cacheKeyFn), (err, result) => err ? reject(err) : parse(result, opt).then(resolve)));
    const rMGet = (keySpace, keys, opt) => new Promise((resolve, reject) => redis.mget(_.map(keys, k => makeKey(keySpace, k, opt.cacheKeyFn)), (err, results) => {
        return err
            ? reject(err)
            : Promise.map(results, r => parse(r, opt)).then(resolve);
    }));
    const rDel = (keySpace, key, opt) => new Promise((resolve, reject) => redis.del(makeKey(keySpace, key, opt.cacheKeyFn), (err, resp) => err ? reject(err) : resolve(resp)));
